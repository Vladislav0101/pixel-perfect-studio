    ...
    <section ref={ref} data-services-section className="section-spacing bg-card">
      <div className="container-custom">
        ...
        >
          ...
          <h2 className="text-headline sm:mt-4">Инструменты <br className='block sm:hidden' /> для роста</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {services.map((service, index) => (
            ...
            >
              <Link
                to={service.link}
                className="group block p-6 sm:p-8 rounded-xl bg-background border border-border card-hover h-full"
              >
                <div className="flex items-start gap-5">
                  <div className="w-10 sm:w-14 h-10 sm:h-14 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0 group-hover:bg-primary/20 transition-colors duration-300">
                    <service.icon className="w-5 sm:w-7 h-5 sm:h-7 text-primary" />
                  </div>
                  <div className="flex-1">
                    <h3 className="font-display text-base sm:text-xl font-bold mb-3 group-hover:text-primary transition-colors duration-300">
                      {service.title}
                    </h3>
                    <p className="text-muted-foreground sm:leading-relaxed mb-4 text-sm sm:text-base">
                      {service.description}
                    </p>
                    <span className="inline-flex items-center text-blue font-semibold text-xs sm:text-sm">
                      Узнать больше
